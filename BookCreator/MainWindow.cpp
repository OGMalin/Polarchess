...
//
	createMenu();
	createStatusbar();
	bookStatus = new QLabel("Positions: , Moves:", this);
	statusBar()->addPermanentWidget(bookStatus);
...
//	v1Splitter->addWidget(openingwindow);
	...
	hSplitter->addWidget(movewindow);
...
//
	...
	HASHKEY key = polyglot.getKey(currentBoard);
//
//	readDB();
//
	boardwindow->setPosition(currentBoard);
...
void MainWindow::createMenu()
{
	...
	homeAct = fileMenu->addAction("Home", this, &MainWindow::home);
	newAct = fileMenu->addAction("New", this, &MainWindow::newBook);
	openAct = fileMenu->addAction("Open", this, &MainWindow::openBook);
	importAct = fileMenu->addAction("Import", this, &MainWindow::import);
	saveAct = fileMenu->addAction("Save", this, &MainWindow::saveBook);
	saveAsAct = fileMenu->addAction("Save as", this, &MainWindow::saveBookAs);
	closeAct = fileMenu->addAction("Close", this, &MainWindow::closeBook);
...
void MainWindow::home()
{
	currentBoard.setStartposition();
	boardwindow->setPosition(currentBoard);
	movewindow->refresh(currentBoard, polyglot);
}
...
{
	QString path = QFileDialog::getSaveFileName(this, "Save Polyglot book", QString(), "bin files (*.bin)");
	if (!path.isEmpty())
	{
		polyglot.filename = path;
		polyglot.save();
	}
...
{
	ImportLC dialog(this);
	if (dialog.exec() == QDialog::Rejected)
		return;

	connect(&dialog, SIGNAL(newPosition(ChessBoard&)), this, SLOT(importPosition(ChessBoard&)));

	dialog.Import(this, &polyglot);

	disconnect(&dialog, nullptr, nullptr, nullptr);
	movewindow->refresh(currentBoard, polyglot);
	showBookStatus();
}

void MainWindow::importPosition(ChessBoard& cb)
{
	boardwindow->setPosition(cb);
}

void MainWindow::showBookStatus()
{
	QString qs;
	QTextStream(&qs) << "Positions: " << polyglot.positions() << ", Moves: " << polyglot.moves();
//	statusBar()->addPermanentWidget(new QLabel(qs,this));
//	statusBar()->showMessage(qs);
	bookStatus->setText(qs);
}
...
//	}
//
	currentBoard.doMove(move, false);
	boardwindow->setPosition(currentBoard);
	movewindow->refresh(currentBoard, polyglot);
}
//
